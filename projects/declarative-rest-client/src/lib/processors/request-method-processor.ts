  ...
  return (annotationArgs: string | RequestMethodArgs) => {
    return (
      ...
      descriptor.value = function (...targetMethodArgs: any[]) {
        ...
        const response = sendRequest.call(
          this,
          requestMethod,
          url,
          body,
        ...
        return applyOnEmitDecorator(
          emitters,
          applyMapDecorator(mapper, annotationArgs, response)
        );
      };
...
function sendRequest(
  ...
  params: HttpParams,
  headers: HttpHeaders,
  options?: { timeoutValue?: number; responseAuthHeaders?: string[] }
  ...
  return addRequestInterceptor.call(this, request).pipe(
    mergeMap((req: HttpRequest<any>) =>
      (this.httpClient as HttpClient).request(req)
    ),
    timeout(options.timeoutValue),
    tap((r: HttpResponse<any>) =>
      options.responseAuthHeaders.forEach((token) =>
        sessionStorage.setItem(token, r.headers.get(token))
      )
    ),
    switchMap(
      (e) =>
        addResponseInterceptor.call(this, e) as Observable<HttpResponse<any>>
    )
  );
}
...
function getRequestMethod(
  method: RequestMethod,
  request: string | RequestMethodArgs
) {
...
function getMetadata(
  ...
  return {
    pathParams: enrichMetadata(
      target[propertyKey + metadataKeySuffix.pathParam],
      methodArgs
    ),
    queryParams: enrichMetadata(
      target[propertyKey + metadataKeySuffix.queryParam],
      methodArgs
    ),
    plainQueryParams: enrichMetadata(
      target[propertyKey + metadataKeySuffix.plainQuery],
      methodArgs
    ),
    body: enrichMetadata(
      target[propertyKey + metadataKeySuffix.body],
      methodArgs
    ),
    plainBody: enrichMetadata(
      target[propertyKey + metadataKeySuffix.plainBody],
      methodArgs
    ),
    header: enrichMetadata(
      target[propertyKey + metadataKeySuffix.header],
      methodArgs
    ),
  };
}
...
function enrichMetadata(
  ...
  return metadata.map((m) =>
    Object.assign({}, m, { value: methodArgs[m.index] || m.value })
  );
}
